...
// CUSTOM TOOLTIP (Z-Index muammosi hal qilindi)
...
export default function HomePage({ data, onNavigate, onTransactionClick, onContextMenu, onAddWallet }: HomePageProps) {
  ...
  // Agar hamyon o'chirilsa va index yo'q joyga tushib qolsa, uni 0 ga qaytaramiz
  useEffect(() => {
    if (currentWalletIndex >= data.wallets.length) {
  ...
  // Active Wallet (Xavfsiz olish)
  const activeWallet = viewMode === 'cards' && data.wallets.length > 0 
      ? data.wallets[Math.min(currentWalletIndex, data.wallets.length - 1)] 
      : null;
  ...
  const handleTouchEnd = (e: React.TouchEvent) => {
    ...
    if (diff > 50) { 
        if (viewMode === 'chart') { setAnimClass('slide-in-right'); setViewMode('cards'); }
             ...
             setAnimClass('slide-in-right'); setCurrentWalletIndex(p => p + 1);
        }
    }
    if (diff < -50) { 
        if (viewMode === 'cards') {
  ...
  return (
    <div className="h-full flex flex-col overflow-y-auto pt-safe px-4 pb-48" onTouchStart={e => touchStartX.current = e.targetTouches[0].clientX} onTouchEnd={handleTouchEnd}>
            ...
            <div className="flex justify-center mb-8 relative">
               <div className="w-[300px] h-[300px] relative">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie data={chartData} innerRadius={90} outerRadius={115} paddingAngle={6} cornerRadius={6} dataKey="value" stroke="none" 
                        onClick={(entry) => {
          ...
          {viewMode === 'cards' && activeWallet && (
            <div className="mb-8 px-2 animate-slideUp">
                ...
                <div onContextMenu={(e) => { e.preventDefault(); onContextMenu(e, activeWallet, 'wallet'); }} 
                    ...
                    style={{ background: `linear-gradient(135deg, rgba(20,30,60,0.95), rgba(10,14,23,0.95))` }}>
                    ...
                    <div className="flex justify-between items-start z-10">
                        <span className="text-gray-300 text-sm font-bold uppercase tracking-widest flex items-center gap-2"><Wallet size={16} className="text-[#00d4ff]"/> {activeWallet.name}</span>
                        <img src="https://raw.githubusercontent.com/muhwezi/react-credit-cards/master/src/images/chip.png" className="w-10 opacity-70"/>
                    </div>
                    <div className="z-10">
                        <h3 className="text-3xl font-mono text-white tracking-widest drop-shadow-md">{activeWallet.balance.toLocaleString()}</h3>
