...
export default function StatsPage({ data, initialFilter, onClearFilter, onTxClick }: Props) {
  const [tab, setTab] = useState<'dashboard' | 'history' | 'ai'>('dashboard'); // AI tab qo'shildi
  ...
  const handleTouchEnd = (e: React.TouchEvent) => {
    ...
    if (diff > 50) {
        ...
        else if(tab === 'history') setTab('ai'); // AI ga o'tish
    } 
    if (diff < -50) {
  ...
  const aiAnalysis = useMemo(() => {
      ...
      // 1. Tejash bo'yicha
      if (savingsRate > 20) insights.push({ icon: 'good', text: `Barakalla! Siz daromadingizning ${savingsRate.toFixed(0)}% qismini tejab qoldingiz. Bu ajoyib natija!` });
      else if (savingsRate > 0) insights.push({ icon: 'neutral', text: `Sizda tejash ko'rsatkichi ${savingsRate.toFixed(0)}%. Xarajatlarni biroz optimallashtirish kerak.` });
      ...
      if(todayTxCount === 0) insights.push({ icon: 'neutral', text: "Bugun hali hech qanday amal kiritmadingiz. Hamyonni tekshirib ko'ring." });
  ...
  const handleExport = async () => {
    ...
    } catch (e: any) { alert("Xatolik: Fayl saqlash uchun ruxsat kerak yoki ilova xotiraga yozolmayapti."); }
  };
    ...
    <div className="h-full flex flex-col bg-[#0a0e17] scroll-area pb-32" onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
       ...
       {tab === 'dashboard' && (
           <div className="px-6 space-y-6 animate-slideUp pb-10">
               ...
               {/* Charts... (Weekly, Pie, Wallet - Oldingi kod kabi) */}
               <div className="bg-[#141e3c] rounded-[24px] p-5 border border-white/5 shadow-xl">
                   ...
                   <div className="h-48"><ResponsiveContainer width="100%" height="100%"><BarChart data={stats.weeklyData}><CartesianGrid strokeDasharray="3 3" stroke="#2a3042" vertical={false}/><XAxis dataKey="name" tick={{fontSize: 10, fill: '#666'}} axisLine={false} tickLine={false}/><ReTooltip contentStyle={{background: '#0a0e17', borderRadius: '10px', border:'none'}}/><Bar dataKey="Kirim" fill="#00d4ff" radius={[4,4,0,0]}/><Bar dataKey="Chiqim" fill="#ff3366" radius={[4,4,0,0]}/></BarChart></ResponsiveContainer></div>
               </div>
               <div className="bg-[#141e3c] rounded-[24px] p-5 border border-white/5 shadow-xl flex flex-col items-center">
                   ...
                   <div className="h-64 w-full"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={stats.pieData} cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="value">{stats.pieData.map((entry, index) => (<Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />))}</Pie><ReTooltip contentStyle={{background: '#0a0e17', borderRadius: '10px', border:'none'}}/><Legend layout="vertical" verticalAlign="middle" align="right" wrapperStyle={{fontSize: '10px'}}/></PieChart></ResponsiveContainer></div>
               </div>
           </div>
       )}

       {/* AI ANALYTICS TAB (YANGI) */}
       {tab === 'ai' && (
           <div className="px-6 space-y-6 animate-slideUp pb-10">
       ...
       {/* HISTORY (Oldingi kod bilan bir xil, faqat Excel tugmasi handleExport ga ulangan) */}
       {tab === 'history' && (
       ...
       {/* Filter Modal oldingi javobdagi bilan bir xil, shu yerda bo'lishi kerak */}
       {isFilterOpen && (
           <div className="fixed inset-0 z-[200] bg-black/90 backdrop-blur-sm flex items-end sm:items-center justify-center animate-slideUp">
               <div className="bg-[#141e3c] w-full max-w-sm h-[80vh] rounded-t-[30px] sm:rounded-[30px] p-6 border-t border-white/10 shadow-2xl overflow-y-auto flex flex-col">
                   ...
                   <div className="space-y-5 flex-1">
                       {/* Filter Inputs (Vaqt, Turi, Hamyon, Kategoriya, Lokatsiya) */}
