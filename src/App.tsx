...
import { Home, BarChart2, Plus, PieChart, Sparkles, User, Lock, Trash2, LogOut, Shield, Fingerprint, Key, Camera } from 'lucide-react';
...
const ProfilePage = ({ data, onUpdateSettings }: { data: AppData, onUpdateSettings: (s: any) => void }) => {
    ...
    // Rasmni o'zgartirish (Hozircha faqat vizual)
    const handleAvatarChange = () => {
        alert("Rasm yuklash tez orada ishga tushadi!");
    };
    ...
    return (
        <div className="p-6 pt-10 animate-slideUp pb-32">
            ...
            <div className="mb-6">
                ...
                <div className="bg-[#141e3c] p-4 rounded-2xl border border-white/5">
                    <div className="flex items-center gap-3 mb-3">
                        <div className="p-2 bg-white/5 rounded-lg"><Key size={18} className="text-[#bb86fc]"/></div>
                        <span className="text-white text-sm font-bold">API Kalit (Gemini/OpenAI)</span>
                    </div>
                    <input 
                        type="password" 
                        placeholder="sk-..." 
                    ...
                    />
                    <p className="text-[10px] text-gray-500 mt-2">Agar bo'sh qoldirilsa, ilova o'zining ichki bazasidan foydalanadi.</p>
                </div>
            </div>
            ...
            <div className="mt-6 p-4 bg-yellow-500/10 border border-yellow-500/20 rounded-xl flex gap-3">
                ...
                <p className="text-yellow-500 text-xs leading-relaxed"><b>Eslatma:</b> Dasturni yangilashda eskisini o'chirib tashlashga to'g'ri kelyapti (Android imzo xatoligi sababli). Bu vaqtinchalik muammo.</p>
            </div>
        </div>
    )
}
...
const LockScreen = ({ correctPin, onUnlock }: { correctPin: string, onUnlock: () => void }) => {
    ...
    const handleInput = (val: string) => {
        ...
        if (newVal.length === correctPin.length) {
            if (newVal === correctPin) onUnlock(); else { setError(true); setTimeout(() => { setInput(''); setError(false); }, 500); }
        }
    }
    return (
...
function App() {
  ...
  const [isLocked, setIsLocked] = useState(!!data.settings?.pinCode);
  ...
  useEffect(() => {
    CapacitorApp.addListener('backButton', () => {
      if (isLocked) return;
      if (isTxModalOpen || isWalletModalOpen || detailTx || contextMenu) { setIsTxModalOpen(false); setIsWalletModalOpen(false); setDetailTx(null); setContextMenu(null); return; }
      if (historyStack.length > 0) { const prev = historyStack[historyStack.length - 1]; setHistoryStack(p => p.slice(0, -1)); setActiveTab(prev); return; }
  ...
  const handleTransactionSave = (txData: Transaction) => {
    let newTx = [...data.transactions]; let newW = [...data.wallets];
    if (editingTx) { const old = data.transactions.find(t => t.id === editingTx.id); if(old) { newW = newW.map(w => w.id === old.walletId ? { ...w, balance: w.balance + (old.type === 'income' ? -old.amount : old.amount) } : w); newTx = newTx.filter(t => t.id !== editingTx.id); } }
    const finalTx = { ...txData, id: txData.id || Date.now().toString() }; newTx.push(finalTx); newW = newW.map(w => w.id === finalTx.walletId ? { ...w, balance: w.balance + (finalTx.type === 'income' ? finalTx.amount : -finalTx.amount) } : w);
    setData({ ...data, transactions: newTx, wallets: newW }); setIsTxModalOpen(false); setEditingTx(null); setDetailTx(null);
  };
  const handleDeleteTx = (id: string) => {
     ...
     setData({ ...data, transactions: data.transactions.filter(t => t.id !== id), wallets: newW }); setContextMenu(null); setDetailTx(null);
  };
  const handleDeleteWallet = (id: string) => {
      ...
      setData({ ...data, wallets: data.wallets.filter(w => w.id !== id), transactions: data.transactions.filter(t => t.walletId !== id) }); setContextMenu(null);
  };
